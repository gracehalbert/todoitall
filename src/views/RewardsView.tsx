...
export default function RewardsView() {
  ...
  return (
    <div>
      ...
      {/* Balance */}
      <div className="bg-gradient-to-br from-green-900/40 to-emerald-900/40 border border-green-800/50 rounded-2xl p-4 mb-4">
        <div className="text-3xl font-bold text-green-300">${totalPoints.toFixed(2)}</div>
        <div className="text-xs text-gray-400 mt-0.5">available to spend</div>
      </div>

      {/* Streaks */}
      {topStreaks.length > 0 && (
        <div className="mb-4">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Current Streaks</h3>
          <div className="space-y-2">
            {topStreaks.map((h) => (
              <div key={h.id} className="flex items-center gap-3 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
      ...
      {/* Rewards list */}
      <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Your Rewards</h3>
      {customRewards.length === 0 && (
        <div className="text-center text-gray-600 py-12">
          <div className="text-4xl mb-2">🎁</div>
          <p>Add rewards to spend your earnings on!</p>
        </div>
      )}
      <div className="space-y-2">
        {customRewards.map((reward) => {
          ...
          return (
            <div key={reward.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center gap-3">
              <span className="text-3xl">{reward.emoji}</span>
              <div className="flex-1 min-w-0">
                <div className="font-medium text-white text-sm">{reward.title}</div>
                {reward.description && <div className="text-xs text-gray-500 mt-0.5">{reward.description}</div>}
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-xs text-green-400 font-semibold">${reward.cost.toFixed(2)}</span>
                  {reward.redeemedCount > 0 && (
                    <span className="text-xs text-gray-500">redeemed {reward.redeemedCount}×</span>
                  )}
                </div>
              </div>
                ...
                {EMOJIS.map((e) => (
                  <button
                    ...
                    className={`text-xl w-9 h-9 rounded-lg border-0 cursor-pointer transition-all ${
                      form.emoji === e ? 'bg-indigo-600 scale-110' : 'bg-gray-800 hover:bg-gray-700'
                    }`}
                  >
                    {e}
                  </button>
