...
export default function RoutinesView() {
  ...
  const handleSave = () => {
    ...
    if (editingId) { updateRoutine(editingId, data) } else { addRoutine(data) }
    setShowModal(false)
  }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const fromIndex = routines.findIndex((r) => r.id === active.id)
    const toIndex = routines.findIndex((r) => r.id === over.id)
    if (fromIndex !== -1 && toIndex !== -1) reorderRoutines(fromIndex, toIndex)
  }
  ...
  const handleStepDragEnd = (routineId: string, event: DragEndEvent) => {
    ...
    if (!routine) return
    const fromIndex = routine.steps.findIndex((s) => s.id === active.id)
    const toIndex = routine.steps.findIndex((s) => s.id === over.id)
...
function SortableStep({ step, completedToday, onToggle }: {
    ...
    <div ref={setNodeRef} style={style} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="p-4">
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-start gap-2">
            <button
              {...attributes} {...listeners}
              className="mt-0.5 flex-shrink-0 text-gray-300 hover:text-gray-500 bg-transparent border-0 cursor-grab active:cursor-grabbing touch-none"
              style={{ fontSize: '1rem', lineHeight: 1, padding: '0 2px' }}
            >
              ⠿
            </button>
      ...
      {!completedToday && expanded && (
        <div className="px-4 pb-4 flex gap-2">
          ...
          {completedCount > 0 && (
            <button onClick={() => onReset(routine.id)} className="text-gray-400 hover:text-gray-600 text-xs bg-transparent border-0 cursor-pointer px-2">
              Reset
            </button>
          )}
        </div>
