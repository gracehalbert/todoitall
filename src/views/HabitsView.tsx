...
export default function HabitsView() {
  ...
  const handleSave = () => {
    ...
    if (editingId) { updateHabit(editingId, data) } else { addHabit(data) }
    setShowModal(false)
  }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const fromIndex = habits.findIndex((h) => h.id === active.id)
    const toIndex = habits.findIndex((h) => h.id === over.id)
    if (fromIndex !== -1 && toIndex !== -1) reorderHabits(fromIndex, toIndex)
  }
...
function HabitCard({
  habit, today, onComplete, onUncomplete, onDelete, onEdit,
}: {
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => onEdit(habit)} className="text-gray-400 hover:text-violet-500 bg-transparent border-0 cursor-pointer text-sm px-1" title="Edit">✎</button>
      ...
      <div className="flex gap-1">
        {last7.map(({ date, done }) => (
          <div key={date} title={date} className="flex-1 h-2 rounded-sm" style={{ backgroundColor: done ? habit.color : '#e9d5ff' }} />
        ))}
      </div>
